...
main_configure_networking() {
    ...
    for entry in "${SYSCTL_SETTINGS[@]}"; do
        ...
        # Skip bridge-nf-call settings in container without br_netfilter
        if [[ "$br_netfilter_available" == false ]] && [[ "$RUNNING_IN_CONTAINER" == true ]] && [[ "$key" == net.bridge.* ]]; then
    ...
    if [[ "$SYSCTL_FILE_CHANGED" == true ]]; then
        # Skip reload in containers without br_netfilter — persisted settings
        # include net.bridge.* which can't be applied at runtime, and individual
        # settings were already applied above
        if [[ "$RUNNING_IN_CONTAINER" == true ]] && [[ "$br_netfilter_available" == false ]]; then
            print_info "Skipping sysctl reload (container without br_netfilter — settings applied individually)"
        else
