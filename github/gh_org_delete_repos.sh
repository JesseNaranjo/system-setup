...
download_script() {
    ...
    if [[ "$DOWNLOAD_CMD" == "curl" ]]; then
        ...
        # the file has already replaced the original on disk. Reading 2 lines
        # via bash `read` also confirms the file is more than a bare shebang;
        # `|| true` lets a 1-line file reach the explicit checks below rather
