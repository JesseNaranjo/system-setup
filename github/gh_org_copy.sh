...
# Resolve script directory once at file scope so cleanup/sweep helpers can find
# in-flight and stale temps adjacent to the live script.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# ============================================================================
# Temp File Cleanup (Defense-in-depth: inline rm-f + EXIT trap + startup sweep)
# ============================================================================
...
print_timestamp() {
    local section="$1"
    printf '%b[%s]%b %s\n' "$GRAY" "$(date +'%Y-%m-%d %H:%M:%S')" "$NC" "$section"
}
...
self_update() {
    ...
    # rather than silently auto-accept via the empty-reply branch. Use the
    # `{ : </dev/tty; }` open(2) probe, NOT `[[ -r /dev/tty ]]` — the latter stays
    # true under setsid while open() fails with ENXIO, so the bare `read </dev/tty`
    # below would abort under set -e. Continue with the unchanged local version.
...
main() {
    # Reap leftover ~*.tmp.?????? files from a prior interrupted run before
    # any new ones are minted. Catch-all glob covers self-update temps.
    sweep_stale_temps '~*.tmp.??????'

    # Check for updates if download tool available. The guard is compared as a
    # STRING: `[[ $v -eq 0 ]]` evaluates both operands as arithmetic, so a
    # `$(…)` smuggled in through the environment would be executed right here.
