...
configure_tmux() {
    ...
    local config_file
    config_file=$(get_tmux_config_file "$scope")

    # Create config file if it doesn't exist (with world-readable permissions)
    if [[ ! -f "$config_file" ]]; then
        print_info "Creating new tmux configuration file: $config_file"
    fi
    create_config_file "$config_file"
    ...
    add_config_if_needed "tmux" "$config_file" "set -g pane-border-format" '"#{?pane_active,#[fg=#ff0051]#[bg=#4aa244]  ●#[fg=#000000],#[fg=#333333]#[bg=#858585]  ○} #{pane_title}  //  #{pane_current_command} "' "pane border format"
    add_config_if_needed "tmux" "$config_file" "set -g status-left" '" Current Pane: #{window_index}.#{pane_index} // "' "session name in status bar"
    add_config_if_needed "tmux" "$config_file" "set -g status-left-length" "0" "status-left max length"
    add_config_if_needed "tmux" "$config_file" "set -g status-right" '"Session: #{session_name} // %a %b %d %I:%M "' "current date and time in status bar"
