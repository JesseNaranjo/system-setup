#!/usr/bin/env bash
# utils-misc.sh — Shared functions for utils scripts

# ── Source guard ───────────────────────────────────────────────────────────────
if [[ -n "${UTILS_MISC_SH_LOADED:-}" ]]; then
    return 0
fi
readonly UTILS_MISC_SH_LOADED=true

set -euo pipefail

# ── Runtime baseline ──────────────────────────────────────────────────────────
# bash 5+ is required and assumed throughout utils/. Nothing here is written to
# degrade to an older shell: prompt_yes_no below uses ${var,,}, dig-all.sh uses
# `declare -A`, services-check.sh uses ${var,,}. macOS still ships bash 3.2 as
# /bin/bash, where each of those fails at RUN time — partway through a run, with
# a message ("bad substitution") that names nothing useful. Every utils/ script
# sources this file, so this is the one place the baseline is asserted; the
# per-script guard that used to live in rsync-over-tunnel.sh is gone.
# Raw printf, not print_error: the print_* helpers are defined below this.
if ((BASH_VERSINFO[0] < 5)); then
    printf 'bash 5+ required (found %s). On macOS: brew install bash\n' "$BASH_VERSION" >&2
    exit 69
fi
